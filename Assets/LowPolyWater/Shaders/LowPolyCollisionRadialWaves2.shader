...
Shader "Trolltunga/LowPolyCollisionRadialWaves 2.0" 
{
	Properties
	{
		...
		_CollisionWaveSpread("Collision wave spread", Float) = 2.0
	...
	SubShader
	{
		...
		Pass
		{
			...
			CGPROGRAM
				...
				float _CollisionWaveSpread;
				vector _CollisionVectors[99];
				...
				vFwdBase vert(VertexInput v)
				{
					...
					for (int i = 0; i < 99; i++)
					{
						...
						{
							collPhase += (waveHeight * ((1.0 - waveState) * distanceToCenter) / (_CollisionWaveSpread * 10 * waveState)) * sin(distanceToCenter - (_CollisionWaveLength * 10 * waveState));
						}
					}
