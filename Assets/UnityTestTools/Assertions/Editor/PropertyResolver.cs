...
namespace UnityTest
{
    ...
    public class PropertyResolver
    {
        ...
        private string[] GetPropertiesAndFieldsFromType(Type type, int level)
        {
			...
			fields.AddRange(type.GetFields().Where(f => !System.Attribute.IsDefined(f, typeof(ObsoleteAttribute))).ToArray());
			fields.AddRange(type.GetProperties().Where(info => info.GetIndexParameters().Length == 0 && !System.Attribute.IsDefined(info, typeof(ObsoleteAttribute))).ToArray());
